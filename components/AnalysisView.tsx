...
import { AnalysisResults, StrategyKey, UploadedImageKeys, User, Trade, UserSettings, SavedTrade, UserUsage, StrategyLogicData, AssetComparisonResult, SavedAssetComparison, ApiConfiguration } from '../types';
...
const AnalysisView: React.FC<AnalysisViewProps> = ({
    ...
    const [tradeToExecute, setTradeToExecute] = React.useState<Trade | null>(null);
    ...
    return (
        <div className="p-4 md:p-6 space-y-6">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                                    {tradesToDisplay.length > 0 ? tradesToDisplay.map((trade, index) => (
                                        <TradeCard
                                            ...
                                            onExecuteTrade={() => setTradeToExecute(trade)}
                                        />
                                    )) : (
            ...
            <TradeExecutionModal
                isOpen={!!tradeToExecute}
                trade={tradeToExecute!}
                onClose={() => setTradeToExecute(null)}
                blofinCredentials={
                    apiConfig.blofinApiKey && apiConfig.blofinSecretKey && apiConfig.blofinPassphrase
                        ? {
                            apiKey: apiConfig.blofinApiKey,
                            secretKey: apiConfig.blofinSecretKey,
                            passphrase: apiConfig.blofinPassphrase
                        }
                        : null
                }
            />
