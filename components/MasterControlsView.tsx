...
import { ALL_PERSISTENT_STORAGE_KEYS, DEMO_TICKERS } from '../constants';
import { getAllEntries, clearStore, setItem } from '../idb';
...
export const MasterControlsView: React.FC<MasterControlsViewProps> = ({
    ...
    const parentStrategyCount = useMemo(() => {
        return (Object.values(strategyLogicData) as StrategyLogicData[]).filter(strat => !strat.parentId).length;
    }, [strategyLogicData]);
    ...
    const handleDarknessChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        onUserSettingsChange('uiDarkness', value);
        window.dispatchEvent(new CustomEvent('settingsChanged', { detail: { uiDarkness: value } }));
    };
...
- The entire response must be ONLY the JSON object, without any markdown formatting.`;
    ...
    const handleCreateStrategyFromText = async (content: string, fileName: string) => {
        ...
        const generatePart = async (step: number, partName: string, systemInstruction: string, retries = 3, isJson = false): Promise<string> => {
            const ai = getAiClient();
         ...
         <div className="space-y-6">
             <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                <h4 className="font-semibold text-white mb-3">Appearance</h4>
                 <div className="space-y-4">
                    {(['uiFontSize', 'headingFontSize', 'dataFontSize', 'chatFontSize'] as const).map(key => {
                        const labels: Record<typeof key, string> = { uiFontSize: 'UI Font Size', headingFontSize: 'Heading Font Size', dataFontSize: 'Data Font Size', chatFontSize: 'Chat Font Size'};
                        return (
                            <div key={key}>
                                 <label className="block font-medium text-sm text-gray-300">{labels[key]}: {userSettings[key]}px</label>
                                 <input type="range" name={key} min="10" max="24" value={userSettings[key]} onChange={e => onUserSettingsChange(key, parseInt(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-yellow-500" />
                            </div>
                        )
                    })}
                     <div>
...
const TextareaField: React.FC<{label:string, name:string, value:string, onChange: any, rows: number, isMono?: boolean}> = ({label,name,value,onChange,rows, isMono}) => (
    <div>
        <label htmlFor={name} className="block text-sm font-medium text-gray-300">{label}</label>
